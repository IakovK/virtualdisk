...
#define VENDORNAME L"VDSoft "
#define MODEL       L"Virtual disk "
...
--*/
{
    ...
    case IRP_MN_REMOVE_DEVICE:
		...
		Bus_KdPrint(DeviceData, BUS_DBG_PNP_TRACE, ("Bus_PDO_PnP: DeviceObject = %p,"
			" DeviceData->ParentFdo = %p, ReportedMissing = %d\n",
			DeviceObject, DeviceData->ParentFdo, DeviceData->ReportedMissing));
		DeviceData->Claimed = FALSE;
        //
        ...
        if (DeviceData->ReportedMissing) {
            ...
            status = Bus_DestroyPdo(DeviceObject, DeviceData);
				if (fdoData->NumPDOs == 0)
					KeSetEvent (&fdoData->CleanupReadyEvent, 0, FALSE);
            break;
...
--*/
{
    ...
    case BusQueryDeviceID:
        ...
        buffer = ExAllocatePoolWithTag (PagedPool, BUS_DEVICE_ID_LENGTH, BUSENUM_POOL_TAG);
        ...
        RtlCopyMemory (buffer, BUS_DEVICE_ID, BUS_DEVICE_ID_LENGTH);
    ...
    case BusQueryInstanceID:
        ...
        RtlStringCchPrintfW(buffer, length/sizeof(WCHAR), L"%02x", DeviceData->SerialNo);
    ...
    case DeviceTextDescription:
        ...
        case 0x00000409 : // English
            if (!Irp->IoStatus.Information) {
                ...
                RtlStringCchPrintfW(buffer, length/sizeof(WCHAR), L"%ws%ws%d", VENDORNAME, MODEL,
                                            DeviceData->SerialNo);
                Bus_KdPrint_Cont (DeviceData, BUS_DBG_PNP_TRACE,
...
--*/
{
    ...
    busInfo->BusTypeGuid = GUID_DEVCLASS_VDISK;
